...
MyFrame5::MyFrame5(wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style) : wxFrame(parent, id, title, pos, size, style)
{
    ...
    // Connect the Run button event to the event handler
    m_button1->Bind(wxEVT_BUTTON, &MyFrame5::OnRunButtonClicked, this);
}
...
auto test_parser(const string& source_code) {
    Lexer lexer(source_code);
    Parser parser(lexer);
    ...
    try {
        ...
        parser.values.push_back(err.what());
        return parser.values;
    }
}

auto run(string pretext) {

    string source_code = "R\"(" + pretext + ")\"";

    std::cout << "Testing lexer:" << std::endl;
    test_lexer(source_code);
    std::cout << "\nTesting parser:" << std::endl;

    return test_parser(pretext);
}

void MyFrame5::OnRunButtonClicked(wxCommandEvent& event)
{
    string text = m_scintilla2->GetText();
    auto values = run(text);
    string label;
    for (const auto& value : values)
        label += value;

    m_staticText10->SetLabel(label); // Display the retrieved text in the static text control
...
public:
    virtual bool OnInit() {
        MyFrame5* frame = new MyFrame5(nullptr, wxID_ANY, "Pascal--");
        frame->Show(true);
        return true;
    }
};
